...
-   By using the `ShowMe()` function, you can generate a **line graph** of your desired data. For example, to plot the [Fertilizer Consumption (FC)]{.underline} for all [10 European countries]{.underline}, simply run the following line of code:\
    `ShowMe(FertilizerConsumption)`

    ![](images/clipboard-3097835763.jpeg)

    As another example, to view the [Wheat Production (WP)]{.underline} graph for a specific country, such as [Germany]{.underline}, use:\
...
As the final example, to see the [Wheat Production (WP)]{.underline} line graph for [10 European countries]{.underline}, run:
